...
/* ── Cost estimate (mirrors _cost_estimate.py) ──────────────────────────── */
const CHARS_PER_SHOT = 280;
const SEGMENTS_PER_SHOT = 2.0;
const SECONDS_PER_SHOT = 240;
const PLANNER_FIXED_S = 90;
const YUAN_PER_SEGMENT = 4.0;
const YUAN_PER_ANCHOR = 1.5;

function estimate(chars) {
  if (!chars) return { shots: 0, segments: 0, wall_min: 0, cost_yuan: 0 };
  const shots = Math.max(3, Math.min(8, Math.round(chars / CHARS_PER_SHOT)));
  const segments = Math.max(shots, Math.round(shots * SEGMENTS_PER_SHOT));
  const wallS = PLANNER_FIXED_S + shots * SECONDS_PER_SHOT;
  const wallMin = Math.max(5, Math.round(wallS / 60));
  const cost = segments * YUAN_PER_SEGMENT + shots * YUAN_PER_ANCHOR + 3 * YUAN_PER_ANCHOR;
  return { shots, segments, wall_min: wallMin, cost_yuan: Math.round(cost * 10) / 10 };
}
...
/* Convert an asset id like "old_father" / "kitchen_prep_counter" into a
   ...
   Used to display memory + ref-chip labels — the manifest's original
   .name / .reference_name fields are Chinese (videorlm planner output)
   and we want pure-English UI in the demo. */
...
const dom = {
  ...
  copyCapabilityBtn: $("copyCapabilityBtn"),
  deleteBtn: $("deleteBtn"),
...
const state = {
  ...
  errorTail: [],
  navStage: "form",
  archiveDeleteToken: null,
  archiveWatchPath: null,
  archivePublic: false,
  tree: null,
  selectedSegId: null,
  ...
  history: null,
  fastForward: false,
  // Backend capabilities probed once at boot (see loadCapabilities)
  capabilities: null,
};
...
function refreshEstimate() {
  const chars = dom.storyInput.value.length;
  const est = estimate(chars);
  dom.estCharsLabel.textContent = `${chars} / 1500 chars`;
  if (chars === 0) {
    dom.estShots.textContent = "·";
    dom.estWall.textContent = "·";
    dom.estCost.textContent = "·";
  } else {
    dom.estShots.textContent = `~${est.shots} shots`;
    dom.estWall.textContent = `~${est.wall_min} min`;
    dom.estCost.textContent = `~¥${est.cost_yuan}`;
  }
  const tooShort = chars < 50;
  const tooLong = chars > 1500;
  const noKey = !dom.keyInput.value.trim();
  dom.submitBtn.disabled = tooShort || tooLong || noKey;
  if (tooShort) dom.submitHint.textContent = "At least 50 characters — planner needs context to split into meaningful shots.";
  else if (tooLong) dom.submitHint.textContent = "Over 1500-char limit, please trim.";
  else if (noKey) dom.submitHint.textContent = "Enter your DashScope API key.";
  else dom.submitHint.textContent = "Will run 15–60 min. You can cancel at any point.";
}
dom.storyInput.addEventListener("input", refreshEstimate);
dom.keyInput.addEventListener("input", refreshEstimate);
dom.toggleKeyBtn.addEventListener("click", () => {
  const showing = dom.keyInput.type === "text";
  dom.keyInput.type = showing ? "password" : "text";
  dom.toggleKeyBtn.textContent = showing ? "Show" : "Hide";
});

async function loadPresets() {
  // Presets feed the (currently hidden) live-generation form; the
  // preset .txt files live in `future/presets/` and aren't shipped
  // in the static deploy. Keep the function as a no-op.
  state.presets = [];
}
...
/* ── Backend capabilities (probed once at boot) ────────────────────────
   Drives feature visibility: when `pipeline_ready=false` the live
   generation form (`<details id="advancedForm">`) stays hidden — clicking
   it would 503 on POST /api/generate anyway. Once videorlm is wired up,
   set MSVE_PIPELINE_READY=1 on the server; the next page load flips
   advancedForm into the UI automatically. */
/* Build version mirrors the ?v= on app.js/app.css. Bump it whenever any
   JSON under /demos/ or /capabilities.json changes. With versioned URLs
   the browser's HTTP cache hits on every reload at the same version,
   versus `cache: "no-store"` which re-fetched the entire payload over
   the wire each time the page loaded (visible as a multi-second "Loading
   demo list…" stall on the dev tunnel). */
const BUILD_VERSION = "20260514-back2";
...
/* Detect the path prefix the site is mounted under so absolute fetches
   ...
   (https://example.com/demo/) or under a subdirectory (e.g.
   https://you.github.io/videorlm-demo/demo/). Convention: the SPA always
   lives at <BASE_PATH>/demo/. */
const BASE_PATH = (() => {
  const m = window.location.pathname.match(/^(.*?)\/demo(?:\/|$)/);
  return m ? m[1] : "";   // "" at host root, "/videorlm-demo" under subpath
})();

async function loadCapabilities() {
  try {
    const r = await fetch(`${BASE_PATH}/demo/capabilities.json?v=${BUILD_VERSION}`);
    if (!r.ok) throw new Error("HTTP " + r.status);
    state.capabilities = await r.json();
  } catch (e) {
    console.warn("capabilities fetch failed", e);
    state.capabilities = { pipeline_ready: false };
  }
  applyCapabilitiesToUI();
}

function applyCapabilitiesToUI() {
  const caps = state.capabilities || { pipeline_ready: false };
  // Show the live-generation form only when the real backend is wired.
  if (dom.advancedForm) {
    dom.advancedForm.hidden = !caps.pipeline_ready;
  }
}
...
async function openHistory(jobId) {
  ...
  try {
    if (rec.demo_id) {
      const r = await fetch(`${BASE_PATH}/demo/data/${encodeURIComponent(rec.demo_id)}/manifest.json?v=${BUILD_VERSION}`);
...
function renderDemoPicker() {
  ...
  if (!state.demos.length) {
    dom.demoPicker.innerHTML = `<p class="demo-picker-loading">No demos — run scripts/build-demo-manifests.py to generate them.</p>`;
    return;
  }
...
async function startDemoReplay(demoId) {
  ...
  // Fetch the manifest. We don't talk to the server's /api/generate or
  // any SSE — the timeline is played entirely client-side with
  // setTimeout, so there is no "connecting…" or job-creation stall.
  let manifest;
  try {
    const r = await fetch(`${BASE_PATH}/demo/data/${encodeURIComponent(demoId)}/manifest.json?v=${BUILD_VERSION}`);
    if (!r.ok) throw new Error("HTTP " + r.status);
...
dom.loadPresetBtn.addEventListener("click", async () => {
  if (!state.presets) await loadPresets();
  if (!state.presets || !state.presets.length) {
    dom.submitHint.textContent = "No sample stories available.";
    return;
  }
  const pick = state.presets[Math.floor(Math.random() * state.presets.length)];
  try {
    const r = await fetch(`/presets/${pick.story_file}`);
    if (!r.ok) throw new Error("preset fetch failed");
    const txt = await r.text();
    dom.storyInput.value = txt.trim().slice(0, 1500);
    refreshEstimate();
    dom.submitHint.textContent = `Loaded sample: ${pick.title}`;
  } catch (e) {
    dom.submitHint.textContent = `Load failed: ${e.message}`;
  }
});

dom.generateForm.addEventListener("submit", async (ev) => {
  ev.preventDefault();
  const story = dom.storyInput.value.trim();
  const key = dom.keyInput.value.trim();
  if (!story || !key) return;
  dom.submitBtn.disabled = true;
  dom.submitHint.textContent = "Creating job…";
  try {
    const r = await fetch("/api/generate", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ story, dashscope_key: key }),
    });
    if (!r.ok) throw new Error(`${r.status}: ${await r.text()}`);
    const j = await r.json();
    state.jobId = j.job_id;
    state.estimate = j.estimate;
    state.replayManifest = null;  // real run → onPlanOk fetches /files
    enterRunning(story);
  } catch (e) {
    dom.submitBtn.disabled = false;
    dom.submitHint.textContent = `Submit failed: ${e.message}`;
  }
});
...
function enterRunning(story) {
  ...
  state.archiveDeleteToken = null;
  state.archiveWatchPath = null;
  state.archivePublic = false;
  ...
  dom.archivePanel.hidden = true;
  if (dom.publishStatus) dom.publishStatus.textContent = "";
  if (dom.deleteStatus) dom.deleteStatus.textContent = "";
  if (dom.watchLink) dom.watchLink.hidden = true;
...
function connectSSE() {
  if (!state.jobId) {
    // Lazy: caller will invoke us again once state.jobId is known.
    return;
  }
  ...
  es.onerror = () => { dom.statusDetail.textContent = "SSE disconnected, retrying…"; };
}
function closeSSE() {
...
/* ── Event handler ─────────────────────────────────────────────────────── */
const STAGE_LABELS = {
  spawn_ok: "Subprocess started",
  shim_installed: "Initialised (shim installed)",
  planner_start: "Planner LLM thinking",
  plan_ok: "Plan skeleton done",
  render_plan_ok: "Render plan ready",
  render_start: "Image DAG started",
};
...
function handleEvent(ev) {
  ...
  // Phase markers are injected by build-demo-manifests.py to slow the
  // replay down between major phases (~5 s pause baked into the next
  // event's t-offset). The handler only updates the status line — it
  // does no DOM mutation, has no validator state effect.
  if (t === "phase_marker") {
  ...
  // ── Lifecycle ──────────────────────────────────────────────────────
  // Per EVENT_SCHEMA.md: spawn_ok / planner_start kick chip ①;
  // shim_installed was demo-only and is removed (real videorlm has no
  // equivalent — see EVENT_SCHEMA.md "Removed / renamed" table).
  ...
  // ── Anchor validator (between image DAG and segments; videorlm has
  //    this implemented today) ────────────────────────────────────────
  ...
  if (t === "archived") {
    state.archiveDeleteToken = ev.delete_token || null;
    state.archiveWatchPath = ev.watch_url || (state.jobId ? `/watch/${state.jobId}` : null);
    state.archivePublic = !!ev.public;
    renderArchivePanel();
  }
...
/* ── plan_ok ─────────────────────────────────────────────────────────────
   ...
   Replay: skeleton lives in state.replayManifest.
   Real:   fetch /files/<jobId>/skeleton.json (best-effort). */
async function onPlanOk(ev) {
  try {
    if (state.replayManifest) {
      preSeedSkeletonFromManifest(state.replayManifest);
      renderShots();
      renderMemory();
      queueDrawLines();
      return;
    ...
    let skel = null;
    try {
      const r = await fetch(`/files/${state.jobId}/skeleton.json`);
      if (r.ok) skel = await r.json();
    } catch (e) {
      console.warn("skeleton fetch failed", e);
    }

    if (skel) {
      preSeedSkeletonFromManifest({ skeleton: skel });
    } else {
      const n = ev.shots || 3;
      state.tree.shots = Array.from({ length: n }, (_, i) => ({
        id: `shot${String(i + 1).padStart(2, "0")}`,
        anchorId: null, anchorUrl: null, status: "pending", leaves: [],
      }));
    }
    renderShots();
    renderMemory();
    ...
    console.error("onPlanOk threw", e);
  }
}

/* ── render_plan_ok ──────────────────────────────────────────────────────
   render_plan returned: paint leaf placeholders under each shot column.
   Replay: use state.replayManifest. Real: fetch render_plan.json. */
async function onRenderPlanOk(ev) {
  try {
    if (state.replayManifest) {
      preSeedRenderPlanFromManifest(state.replayManifest);
      renderShots();
      queueDrawLines();
      return;
    }
  } catch (e) {
    console.error("onRenderPlanOk replay path threw", e);
  }

  let plan = null;
  try {
    const r = await fetch(`/files/${state.jobId}/render_plan.json`);
    if (r.ok) plan = await r.json();
  } catch (e) {
    console.warn("render_plan fetch failed", e);
  }
  if (!plan) { queueDrawLines(); return; }

  // Anchors → shots
  for (const a of (plan.boundary_anchors || [])) {
    const aId = a.id || "";
    const infer = aId.replace(/^a\d+_/, "").replace(/_start$/, "");
    let shot = state.tree.shots.find((s) => s.id === infer || s.id.endsWith(infer) || infer.endsWith(s.id));
    if (!shot && a.shot_id) shot = state.tree.shots.find((s) => s.id === a.shot_id);
  ...
  const stillUnmapped = (plan.boundary_anchors || []).filter((a) => !state.tree.shots.find((s) => s.anchorId === a.id));
  let j = 0;
  for (const shot of state.tree.shots) {
    if (!shot.anchorId && stillUnmapped[j]) { shot.anchorId = stillUnmapped[j].id; j++; }
  }
  ...
  for (const segId in segs) {
    ...
    segByShot[sid].push({
      ...
      url: null,
      prompt: flf2v.prompt || seg.prompt || "",
      refs: extractRefs(flf2v, seg),
      backend: flf2v.kind || seg.kind || "segment_r2v",
      kind: flf2v.kind || seg.kind || "segment_r2v",
      duration_s: flf2v.duration_s || seg.duration_s || 0,
      transition_mode: extractTransitionMode(seg, segId),
      order: parseInt((/_(\d+)$/.exec(segId) || [, "0"])[1], 10),
    });
  }
  for (const shot of state.tree.shots) {
...
function onFinalDone(ev) {
  ...
  if (dom.rootFinalVideo) dom.rootFinalVideo.src = ev.url || `/files/${state.jobId}/run/final.mp4`;
}
...
function enterFinal(ev) {
  ...
  if (!url) return;
  let playUrl = url;
  if (state.jobId && state.archiveDeleteToken) playUrl = `/files/_archive/${state.jobId}/final.mp4`;
  dom.finalVideo.src = playUrl;
  dom.downloadLink.href = playUrl;
  if (ev.total_s != null) {
...
function renderArchivePanel() {
  if (!state.archiveDeleteToken || !state.jobId) {
    if (dom.archivePanel) dom.archivePanel.hidden = true;
    return;
  }
  const origin = window.location.origin;
  const watchPath = state.archiveWatchPath || `/watch/${state.jobId}`;
  const capabilityUrl = `${origin}${watchPath}?delete_token=${state.archiveDeleteToken}`;
  const publicWatchUrl = `${origin}${watchPath}`;
  dom.archivePanel.hidden = false;
  if (dom.capabilityUrl) dom.capabilityUrl.value = capabilityUrl;
  if (dom.watchLink) {
    dom.watchLink.href = publicWatchUrl;
    dom.watchLink.hidden = false;
  }
  if (dom.publishBtn) {
    dom.publishBtn.disabled = false;
    dom.publishBtn.textContent = state.archivePublic ? "Make private" : "Publish to gallery";
  }
  if (dom.publishStatus) {
    dom.publishStatus.textContent = state.archivePublic
      ? "Currently public — anyone with the URL can find it in the gallery."
      : "Currently private — only people with the URL above can view it.";
    dom.publishStatus.className = "archive-status" + (state.archivePublic ? " ok" : "");
  }
}

dom.copyLinkBtn.addEventListener("click", async () => {
  let url = dom.downloadLink.href;
  if (state.jobId && state.archiveWatchPath) url = `${window.location.origin}${state.archiveWatchPath}`;
  if (!url) return;
  try {
...
if (dom.publishBtn) {
  dom.publishBtn.addEventListener("click", async () => {
    if (!state.jobId || !state.archiveDeleteToken) return;
    dom.publishBtn.disabled = true;
    const newPublic = !state.archivePublic;
    try {
      const r = await fetch(`/api/archive/${state.jobId}/publish`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ delete_token: state.archiveDeleteToken, public: newPublic }),
      });
      if (!r.ok) throw new Error(`${r.status}: ${await r.text()}`);
      state.archivePublic = newPublic;
      renderArchivePanel();
      if (dom.publishStatus) {
        dom.publishStatus.textContent = newPublic ? "Public · visible in the gallery" : "Made private";
        dom.publishStatus.className = "archive-status ok";
      }
    } catch (e) {
      if (dom.publishStatus) {
        dom.publishStatus.textContent = `failed: ${e.message}`;
        dom.publishStatus.className = "archive-status err";
      }
      dom.publishBtn.disabled = false;
    }
  });
}
if (dom.copyCapabilityBtn) {
  dom.copyCapabilityBtn.addEventListener("click", async () => {
    const val = dom.capabilityUrl ? dom.capabilityUrl.value : "";
    if (!val) return;
    try {
      await navigator.clipboard.writeText(val);
      dom.copyCapabilityBtn.textContent = "Copied";
      setTimeout(() => { dom.copyCapabilityBtn.textContent = "Copy management link"; }, 1500);
    } catch (_) { dom.copyCapabilityBtn.textContent = "Copy failed"; }
  });
}
if (dom.deleteBtn) {
  dom.deleteBtn.addEventListener("click", async () => {
    if (!state.jobId || !state.archiveDeleteToken) return;
    if (!confirm("Permanently delete this run from the archive?？This cannot be undone.")) return;
    dom.deleteBtn.disabled = true;
    try {
      const r = await fetch(`/api/archive/${state.jobId}?token=${encodeURIComponent(state.archiveDeleteToken)}`, { method: "DELETE" });
      if (!r.ok) throw new Error(`${r.status}: ${await r.text()}`);
      if (dom.deleteStatus) { dom.deleteStatus.textContent = "Deleted. Returning to form…"; dom.deleteStatus.className = "archive-status ok"; }
      setTimeout(() => { state.jobId = null; state.archiveDeleteToken = null; showView("form"); setNav("form"); }, 1500);
    } catch (e) {
      if (dom.deleteStatus) { dom.deleteStatus.textContent = `failed: ${e.message}`; dom.deleteStatus.className = "archive-status err"; }
...
dom.cancelBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    dom.statusDetail.textContent = "Cancel request sent…";
  } catch (_) { dom.statusDetail.textContent = "Cancel failed — subprocess may have already exited."; }
});
